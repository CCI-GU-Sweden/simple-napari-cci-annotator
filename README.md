...
For segmentation, the canonical TIFF mask remains authoritative. YOLO polygon rows are generated invisibly only inside the run snapshot. A connected instance may contain holes: the exporter uses its largest enclosing contour because YOLO rows cannot encode interior holes, then measures the resulting mask→polygon→mask IoU. Validation rejects disconnected IDs, missing/cross-class metadata, invalid or degenerate contours, round-trip IoU below `0.90`, objects in synthetic padding, size/provenance mismatches, and source-group leakage. Objects touching the image boundary are supported. The export manifest records per-class counts, image/label/mask checksums, and minimum/mean round-trip IoU. Empty reviewed masks produce valid empty `.txt` labels. Ultralytics must report task `segment`; a successful `best.pt` is copied to `project/models/<run-name>.pt` like detection training.
