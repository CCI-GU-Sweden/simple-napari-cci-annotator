...
For a segmentation project, loading an image creates or reloads one editable `yolo_instances` napari Labels layer. Polygons are not shown. Select a project class, then use **New Mask Instance** to allocate an ID and paint it. The nearby actions apply a class, delete an instance, merge entered IDs into the selected ID, split a disconnected ID into 4-connected objects, or explicitly keep only the component with the largest bounding-box area. **Remove <5 px** explicitly deletes instances with a total area of 1–4 pixels (a 5-pixel instance is retained). **Collapse Instance IDs** removes metadata entries whose IDs no longer have pixels and deterministically renumbers the surviving IDs to `1..N`; class assignments are preserved. These bulk actions also work in a movable segmentation crop. Per-class counts report only instances that still have pixels, and selected-instance details remain visible.
...
For segmentation, the canonical TIFF mask remains authoritative. YOLO polygon rows are generated invisibly only inside the run snapshot. A connected parent may appear as a donut in the exclusive instance-ID map when another annotated instance occupies its hole—for example, a nucleolus inside a nucleus. When every pixel of a hole belongs to another instance, the exporter treats those IDs as nested children, fills the parent's exterior for export, and writes overlapping parent/child YOLO polygons. The original exclusive IoU and the nesting-adjusted round-trip IoU are both retained. Unexplained background holes still count as topology loss and must satisfy the `0.90` threshold. Validation also rejects disconnected IDs, missing/cross-class metadata, invalid contours, objects in synthetic padding, size/provenance mismatches, and source-group leakage. Objects touching the image boundary are supported. The export manifest records nested relationships, per-class counts, image/label/mask checksums, and IoU summaries. Empty reviewed masks produce valid empty `.txt` labels. Ultralytics must report task `segment`; a successful `best.pt` is copied to `project/models/<run-name>.pt` like detection training.
