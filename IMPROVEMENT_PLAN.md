...
The plugin should provide a simple project-setup GUI for channel selection and image normalization, with a preview and a plain-language explanation of each option. The user is responsible for choosing a scientifically appropriate mapping.

The same deterministic conversion must be used when saving training images and when predicting them. Store the channel mapping and normalization settings in `project.yaml` and every run manifest. These settings can be edited while the project contains no saved images, but become **immutable when the first annotation image is committed**. On every later import/save, validate compatibility and refuse data requiring a different conversion. To change normalization or channel mapping, create a new project (a future explicit project-migration tool may reprocess all source images, but must never do so silently).
...
3. **Project-wide image conversion:** project setup provides simple channel-selection and normalization controls with a preview. The user is responsible for the scientific choice. The settings are locked after the first annotation image is saved and cannot change within that project.
