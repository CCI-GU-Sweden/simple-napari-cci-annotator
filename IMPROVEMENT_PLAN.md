...
**Implementation status: first usable path implemented; real Ultralytics CPU smoke and pinned-repository golden comparison remain.** A task-aware builder validates canonical triples, uses the stable audited source-group split, preserves empty reviewed masks, and copies paired RGB/mask/instance artifacts into the immutable run. Exterior polygons are derived per instance only inside that run and must pass a `0.90` raster round-trip IoU threshold. Connected masks with holes use their largest enclosing contour; YOLO cannot represent the holes, so material topology loss is caught by that IoU threshold rather than by rejecting every multi-contour mask. Image-edge objects are supported through padded contour tracing. The export manifest records per-class counts, checksums, discarded-component count, and IoU summaries. Training rejects a non-segmentation checkpoint, records task/class lineage, preserves Ultralytics outputs, and promotes `best.pt` to the project model folder. Synthetic multi-class, empty-mask, holed-mask, border-mask, polygon, grouping, and widget crop fixtures are present.
